...
void inorderTraversal(node *root)
{
  ...
  for (i = 0; i < root->num_keys; i++)
  {
    if (!root->is_leaf)
    {
      inorderTraversal((node *)root->pointers[i]);
    }
...
void preorderTraversal(node *root)
{
  ...
  for (i = 0; i < root->num_keys; i++)
  {
    ...
    if (!root->is_leaf)
    {
      preorderTraversal((node *)root->pointers[i]);
    }
  }
  if (!root->is_leaf)
  {
...
void postorderTraversal(node *root)
{
  ...
  if (!root->is_leaf)
  {
    for (i = 0; i < root->num_keys; i++)
    {
      postorderTraversal((node *)root->pointers[i]);
    }
  }
  for (i = 0; i < root->num_keys; i++)
  {
...
for (int i = keyIndex; i < leaf->num_keys - 1; i++) {
leaf->keys[i] = leaf->keys[i + 1];
leaf->pointers[i] = leaf->pointers[i + 1];
}
leaf->num_keys--;
...
node *deleteFromBPlusTree(node *root, int key) {
if (root == NULL) {
...
int main()
{
  ...
  root = NULL;
  printf("inserting keys 5, 15, 25, 35, 45 with values 33, 21, 31, 41, 10\n");
  ...
  root = deleteFromBPlusTree(root, 15);
  ...
  postorderTraversal(root);
  printf("\n");
  printf("serching key 25\n");
  findAndPrint(root, 25, instruction = 'a');
}
